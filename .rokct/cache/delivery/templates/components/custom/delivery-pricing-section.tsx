...
// The delivery platform storefront's pricing section, registered in
// base_sdk's page-sections registry (components/custom/landing/page-sections.ts).
// It renders the plan rows the page prefetched through this SDK's plans
// query (landing/delivery-plans-query.ts: the control site's Subscription
// Plan catalog, `plan_category` paas) - name, price, term, trial and, when
...
// control site unreachable, or no paas plan seeded), so the host drops
...
//
// The PaaS plan fixtures' feature lines carry inline markup (the first
// line wraps its "White Label" tag in a styled <span>); this section draws
// feature lines as text, so the markup is stripped and the words kept.
...
/** "PaaS (Monthly)" -> "PaaS": the term is shown separately. */
...
/** A fixture feature line as plain words: tags dropped, whitespace folded. */
function featureText(feature: unknown): string {
  if (typeof feature !== "string") return "";
  return feature.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim();
}
...
export function DeliveryPricingSection({ id, plans }: PageSectionProps) {
  const config = DELIVERY_PAGE_SECTIONS.pricing;
  const { labels } = config;
