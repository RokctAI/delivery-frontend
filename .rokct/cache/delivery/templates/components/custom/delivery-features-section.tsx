...
// The delivery platform storefront's features section, registered in
// base_sdk's page-sections registry (components/custom/landing/page-sections.ts):
// what the platform does, one card per capability, copy (and the source of
// each claim) in landing/delivery-page-sections.ts. Colours route through
// the shell's theme tokens (primary, zinc surfaces), never a literal brand
// colour.
...
export function DeliveryFeaturesSection({ id }: { id?: string }) {
  const config = DELIVERY_PAGE_SECTIONS.features;
...
export default DeliveryFeaturesSection;
